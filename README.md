...
Do not open `apps/*/*.html` directly with `file://`. The app uses same-origin `/api`, `/shared`, and `/client`, `/admin`, `/owner` routes. Run the server from the repository root:
...
Then open `http://localhost:8787/login` in a browser and use DevTools device emulation for mobile viewport testing. This keeps the same routing and asset behavior used by deployment.
