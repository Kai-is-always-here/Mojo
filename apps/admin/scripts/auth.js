...
const savedEmail=localStorage.getItem('bo_admin_login_email');
...
f?.addEventListener('submit',async e=>{e.preventDefault();try{const r=await fetch(API+'/auth/login',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({email:f.email.value.trim(),password:f.password.value,role:'admin'})});const d=await r.json();if(!r.ok)throw Error(d.error||'Login failed');saveSession(d,!!remember?.checked);if(remember?.checked)localStorage.setItem('bo_admin_login_email',f.email.value.trim());else localStorage.removeItem('bo_admin_login_email');location.href='index.html'}catch(e){const m=document.querySelector('#msg');m.textContent=e.message;m.className='auth-message notice error'}});
