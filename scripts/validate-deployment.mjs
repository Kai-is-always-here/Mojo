...
for (const file of walk(root)) {
  ...
  if (path.resolve(file) !== validatorFile && ['.js','.mjs','.html','.css','.yml','.yaml','.json'].includes(ext) && text.includes('localhost:8787')) errors.push(`Localhost dependency: ${path.relative(root, file)}`);
}
